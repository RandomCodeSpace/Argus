    ...
    <div className="mcp-view">
      <div className="mc-status-bar">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.72rem', color: 'var(--text-dim)' }}>
          ...
          <code style={{ fontFamily: 'ui-monospace, monospace', padding: '0.15rem 0.4rem', borderRadius: 4, background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}>{window.location.origin}/mcp</code>
        </div>
        <span className="mc-badge">HTTP Streamable MCP · JSON-RPC 2.0</span>
        <div style={{ marginLeft: 'auto' }}>
          <button className="mc-btn-icon" onClick={() => void connect()}><RefreshCw size={12} /> Reconnect</button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '1rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '0.8rem', alignContent: 'start' }}>
        {status === 'error' && <div className="card" style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '2.5rem 1rem' }}><div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#ef4444', marginBottom: '0.45rem' }}>Connection failed</div><div style={{ color: 'var(--text-muted)', marginBottom: '0.85rem' }}>{error || 'Could not reach the MCP endpoint.'}</div><code>MCP_ENABLED=true</code></div>}
        {status === 'connected' && tools.map((tool, index) => <ToolCard key={tool.name} tool={tool} index={index} onCall={(next) => setCallTool(tools[next])} onRPC={(next) => setRpcTool(tools[next])} />)}
      </div>
